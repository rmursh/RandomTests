...
public:
	...
	public:
		...
		void setRS()
		{
			_recursionStack = true;
		}

		void resetRS()
		{
			_recursionStack = false;
		}
...
private:
	...
	void countNodes(Node* parent)
	{
		...
		{
			Node* n = parent->child(i);
			countNodes(n);
		}
	}

	bool isCyclic(Node* parent)
	{
		if(!parent->hasBeenVisited())
		{
			parent->setVisited();
			...
			{
				Node* n = parent->child(i);
				if(!n->hasBeenVisited()&& isCyclic(n))
				{
					return true;
				}
				else if (n->isPartOfRecursionStack())
				{
					return true;
				}
			}
...
public:
	...
	public:
		...
		iterator_dfs& operator ++ () 
		{
			...
			//given all paths draw a tree in cli c++
	...
	void resetVisitedAll(Node* parent)
	{
		if(parent->hasBeenVisited())
		{
			parent->resetVisited();
